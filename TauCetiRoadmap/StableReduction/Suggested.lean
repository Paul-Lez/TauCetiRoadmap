...
There are no compiled target signatures yet. At the pinned Mathlib commit, the vocabulary
needed to state them faithfully does not exist: families of curves, at-worst-nodal morphisms,
relative dualizing sheaves, stable pointed families, stable maps, models over a DVR, and
finite DVR extensions are all roadmap targets. Introducing placeholder `Prop` definitions
here would make the summit vacuous, so signatures will be added only as those types land in
Tau Ceti.
